...
use hwcodec::{
    common::{DataFormat, Driver, MAX_GOP, H264, H265},
    vram::{
...
impl VRamEncoder {
    ...
    pub fn bitrate(fmt: DataFormat, width: usize, height: usize, ratio: f32) -> u32 {
        crate::hwcodec::HwRamEncoder::calc_bitrate(width, height, ratio, fmt == H264)
    }
...
impl VRamDecoder {
    ...
    pub fn possible_available_without_check() -> (bool, bool) {
        ...
        (
            v.iter().any(|d| d.data_format == H264),
            v.iter().any(|d| d.data_format == H265),
        )
    }
