...
use hwcodec::{
    common::{
        DataFormat, Quality, RateControl, H264, H265, HWCODEC_ERR_HEVC_COULD_NOT_FIND_POC,
        Quality_Default, RC_CBR,
    },
    ffmpeg::{AVPixelFormat, AV_PIX_FMT_NV12, AV_PIX_FMT_YUV420P},
    ffmpeg_ram::{
...
const DEFAULT_PIXFMT: AVPixelFormat = AV_PIX_FMT_NV12;
pub const DEFAULT_FPS: i32 = 30;
const DEFAULT_GOP: i32 = i32::MAX;
const DEFAULT_HW_QUALITY: Quality = Quality_Default;
pub const ERR_HEVC_POC: i32 = HWCODEC_ERR_HEVC_COULD_NOT_FIND_POC as i32;
...
impl EncoderApi for HwRamEncoder {
    ...
    fn encode_to_message(&mut self, input: EncodeInput, ms: i64) -> ResultType<VideoFrame> {
        ...
        if frames.len() > 0 {
            ...
            match self.format {
                H264 => vf.set_h264s(frames),
                H265 => vf.set_h265s(frames),
    ...
    fn yuvfmt(&self) -> crate::EncodeYuvFormat {
        let pixfmt = if self.pixfmt == AV_PIX_FMT_NV12 {
            Pixfmt::NV12
        } else {
            Pixfmt::I420
        };
...
impl HwRamDecoderImage<'_> {
    ...
    pub fn to_fmt(&self, rgb: &mut ImageRgb, i420: &mut Vec<u8>) -> ResultType<()> {
        ...
        match frame.pixfmt {
            ...
            _ => bail!("unsupported pixfmt: {:?}", frame.pixfmt),
        }
        Ok(())
    }
}
...
fn get_mime_type(codec: DataFormat) -> &'static str {
    use hwcodec::common::{AV1, H264, H265, VP8, VP9};
