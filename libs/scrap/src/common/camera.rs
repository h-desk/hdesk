...
impl Cameras {
    ...
    fn create_camera(index: &CameraIndex) -> ResultType<Camera> {
        ...
        let requested = RequestedFormat::with_formats(
            format_type,
            &[FrameFormat::YUYV, FrameFormat::NV12, FrameFormat::RAWRGB],
        );
        let result = Camera::new(index.clone(), requested);
        match result {
            Ok(camera) => Ok(camera),
            Err(primary_err) => {
                let fallback = Camera::new(
                    index.clone(),
                ...
                match fallback {
                    Ok(camera) => Ok(camera),
                    Err(e) => bail!(
                        "create camera{} error: preferred non-MJPEG formats failed: {}; fallback failed: {}",
                        index,
                        primary_err,
                        e
                    ),
...
impl TraitCapturer for CameraCapturer {
    ...
    fn frame<'a>(&'a mut self, _timeout: std::time::Duration) -> std::io::Result<Frame<'a>> {
        ...
        match self.camera.frame() {
            Ok(buffer) => {
                if matches!(buffer.source_frame_format(), FrameFormat::MJPEG) {
                    return Err(io::Error::new(
                        io::ErrorKind::Other,
                        "Camera frame decode error: MJPEG camera frames are disabled in this build",
                    ));
                }
