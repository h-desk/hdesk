...
fn resolve_ui_page(page: &str) -> Option<String> {
    fn candidate_url(root: &Path, page: &str) -> Option<String> {
        let path = root.join("src").join("ui").join(page);
        if path.exists() {
    ...
    if let Ok(current_dir) = std::env::current_dir() {
        if let Some(url) = candidate_url(&current_dir, page) {
            return Some(url);
        }
    }
    ...
    if let Ok(exe) = std::env::current_exe() {
        if let Some(exe_dir) = exe.parent() {
            ...
            roots.extend(exe_dir.ancestors().map(|p| p.to_path_buf()));
            roots.push(PathBuf::from(env!("CARGO_MANIFEST_DIR")));
            for root in roots {
                if let Some(url) = candidate_url(&root, page) {
                    return Some(url);
                }
            }
        }
...
pub fn start(args: &mut [String]) {
    ...
    #[cfg(not(feature = "inline"))]
    if let Some(url) = resolve_ui_page(page) {
        frame.load_file(&url);
    } else {
        log::error!("Failed to locate sciter UI page: {}", page);
        frame.load_html(
            format!(
                "<html><body style='font-family:sans-serif;padding:20px'>Missing UI page: {}</body></html>",
                page
            )
            .as_bytes(),
            Some(page),
