...
impl Client {
    ...
    #[inline]
    fn relay_stream_type() -> &'static str {
        if use_ws() {
            "WebSocket"
        } else {
            "Relay"
    ...
    #[inline]
    fn should_hedge_relay(
        direct_failures: i32,
        peer_nat_type: NatType,
        has_udp_socket_nat: bool,
        has_udp_socket_v6: bool,
    ) -> bool {
        direct_failures > 0
            || peer_nat_type == NatType::SYMMETRIC
            || (!has_udp_socket_nat && !has_udp_socket_v6)
    }

    #[inline]
    fn relay_fallback_delay_ms(
        connect_timeout: u64,
        punch_time_used: u64,
        direct_failures: i32,
        has_udp_socket_nat: bool,
        ...
        let mut delay = if direct_failures > 0 { 700 } else { 1200 };
        if has_udp_socket_nat {
            delay += 200;
        }
        if has_udp_socket_v6 {
            delay += 300;
        }
        delay = delay.max(punch_time_used.saturating_mul(2).clamp(600, 1800));
        let upper = connect_timeout.saturating_sub(100).max(300).min(2500);
        delay.clamp(300, upper)
    }
    ...
    async fn connect(
        ...
        if interface.is_force_relay() {
            if relay_server.is_empty() {
                bail!("Relay server is unavailable");
            }
            log::info!("force relay enabled, skip direct connection attempts");
            ...
            .map_err(|e| {
                ...
                anyhow!("Failed to connect via relay server: {}", e)
            })?;
            let typ = Self::relay_stream_type();
            log::info!(
            ...
            return Ok((conn, false, pk, None, typ));
        }

        let has_udp_socket_nat = udp_socket_nat.is_some();
        let has_udp_socket_v6 = udp_socket_v6.is_some();
        let hedge_relay = !relay_server.is_empty()
            && Self::should_hedge_relay(
                direct_failures,
                peer_nat_type,
                has_udp_socket_nat,
                has_udp_socket_v6,
        ...
        if hedge_relay {
            let relay_delay = Self::relay_fallback_delay_ms(
                connect_timeout,
                punch_time_used,
                direct_failures,
                has_udp_socket_nat,
            ...
            log::info!(
                "peer address: {}, direct timeout: {}, relay hedge delay: {}",
                peer,
                connect_timeout,
                relay_delay
            );
        ...
        let mut direct_future = Box::pin(async move {
            match select_ok(connect_futures).await {
                Ok(conn) => Ok((conn.0 .0, conn.0 .1, conn.0 .2)),
                Err(e) => Err(e),
            }
        ...
        let ((mut conn, kcp, typ), direct) = if relay_server.is_empty() {
            (direct_future.as_mut().await?, true)
        } else if !hedge_relay {
            match direct_future.as_mut().await {
                Ok(result) => (result, true),
                Err(direct_err) => {
                    log::info!("Direct connection failed, switching to relay: {direct_err}");
                    ...
                    .map_err(|relay_err| {
                        interface.update_direct(Some(false));
                        anyhow!(
                            "Failed to make direct connection: {direct_err}; relay fallback failed: {relay_err}"
                        )
                    })?;
                    ((conn, None, Self::relay_stream_type()), false)
                }
            }
        } else {
            let relay_delay = Self::relay_fallback_delay_ms(
            ...
            match timeout(relay_delay, direct_future.as_mut()).await {
                Ok(Ok(result)) => (result, true),
                Ok(Err(direct_err)) => {
                    log::info!(
                        "Direct connection failed before relay hedge fired, switching to relay: {direct_err}"
                    );
                    let conn = Self::request_relay(
                        peer_id,
                        relay_server.to_owned(),
                ...
                Err(_) => {
                    enum ConnectRaceOutcome {
                        Direct(ResultType<(Stream, Option<KcpStream>, &'static str)>),
                        Relay(ResultType<(Stream, Option<KcpStream>, &'static str)>),
                    }

                    log::info!(
                        "Direct connection is still pending after {} ms, starting relay fallback",
                        relay_delay
                    );
                    ...
                    let mut relay_future = Box::pin(async move {
                        let conn = Self::request_relay(
                            peer_id,
                            relay_server,
                            &rendezvous_server,
                        ...
                        .await?;
                        Ok((conn, None, Self::relay_stream_type()))
                    });
                    match tokio::select! {
                        res = direct_future.as_mut() => ConnectRaceOutcome::Direct(res),
                        res = relay_future.as_mut() => ConnectRaceOutcome::Relay(res),
                    } {
                        ConnectRaceOutcome::Direct(Ok(result)) => (result, true),
                        ConnectRaceOutcome::Relay(Ok(result)) => (result, false),
                        ConnectRaceOutcome::Direct(Err(direct_err)) => {
                            let relay_result = relay_future.as_mut().await.map_err(|relay_err| {
                                interface.update_direct(Some(false));
                                anyhow!(
                                    "Failed to make direct connection: {direct_err}; relay fallback failed: {relay_err}"
                                )
                        ...
                        ConnectRaceOutcome::Relay(Err(relay_err)) => {
                            let direct_result =
                                direct_future.as_mut().await.map_err(|direct_err| {
                                    interface.update_direct(Some(false));
                                    anyhow!(
                                        "Relay fallback failed: {relay_err}; direct connection also failed: {direct_err}"
                                    )
                                })?;
                            (direct_result, true)
                        }
