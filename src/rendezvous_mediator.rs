...
async fn direct_server(server: ServerPtr) {
    ...
    let mut port = 0;
    // Spawn a supplementary IPv6-only listener so clients can connect via IPv6 even when
    // listen_any falls back to IPv4-only (dual-stack bind failure on some Windows configs).
    {
        let server_v6 = server.clone();
        tokio::spawn(async move {
            direct_server_v6(server_v6).await;
        });
    }
    loop {
...
/// Supplementary IPv6-only direct server: allows IPv6 clients (e.g. HarmonyOS phone) to TCP
/// connect to port 21118 even when the main `listen_any` falls back to IPv4-only.
/// On systems where the dual-stack bind works, [::]:21118 will be occupied and this bind
/// will fail silently — that's fine, the dual-stack socket already handles IPv6.
async fn direct_server_v6(server: ServerPtr) {
    use tokio::net::TcpListener;
    use std::net::{IpAddr, Ipv6Addr, SocketAddr};
    let port = get_direct_port();
    let addr = SocketAddr::new(IpAddr::V6(Ipv6Addr::UNSPECIFIED), port as u16);
    let std_l = match std::net::TcpListener::bind(addr) {
        Ok(l) => l,
        Err(e) => {
            log::debug!("direct_server_v6: bind [::]:{}  failed (dual-stack may cover it): {}", port, e);
            return;
        }
    };
    ...
    let l = match TcpListener::from_std(std_l) {
        Ok(l) => l,
        Err(e) => { log::error!("direct_server_v6: from_std failed: {}", e); return; }
    };
    log::info!("direct_server_v6: listening on [::]:{}  (IPv6-only)", port);
    loop {
        match hbb_common::timeout(1000, l.accept()).await {
            Ok(Ok((stream, addr))) => {
                stream.set_nodelay(true).ok();
                log::info!("direct_server_v6: connection from {}", addr);
            ...
            Ok(Err(e)) => { log::error!("direct_server_v6: accept error: {}", e); }
