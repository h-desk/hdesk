...
// ─── EditableFocus service ────────────────────────────────────────────────────
// Polls the PC for the currently focused editable element and sends
// EditableFocusHint to connected clients so they can auto-open keyboard
// and adjust their viewport to show the caret/editor area.

pub const NAME_EDITABLE_FOCUS: &'static str = "editable_focus";
...
struct StateEditableFocus {
    ...
    idle_counter: u32,
    last_observed_click_seq: u64,
    /// Last known foreground window handle — used to detect app switch even
    /// when two maximized windows have identical rects.
    last_foreground_hwnd: isize,
}

#[derive(Clone, PartialEq)]
struct RecentEditableHint {
    hint: crate::platform::EditableFocusHintInfo,
    updated_at: Instant,
}

#[derive(Clone)]
struct EditableClickIntent {
    seq: u64,
...
fn rect_contains_point_with_margin(
    ...
    editable_rect_has_area(rect)
        && x >= rect.0 - margin_x
        && x <= rect.0 + rect.2 + margin_x
        && y >= rect.1 - margin_y
        && y <= rect.1 + rect.3 + margin_y
}
...
fn rect_contains_rect_with_margin(
    ...
    editable_rect_has_area(outer)
        && editable_rect_has_area(inner)
...
fn editable_windows_match(a: (i32, i32, i32, i32), b: (i32, i32, i32, i32)) -> bool {
    if !editable_rect_has_area(a) || !editable_rect_has_area(b) {
        return false;
    }
    let same_origin = (a.0 - b.0).abs() <= EDITABLE_PROXY_WINDOW_TOL
        && (a.1 - b.1).abs() <= EDITABLE_PROXY_WINDOW_TOL
        && (a.2 - b.2).abs() <= EDITABLE_PROXY_WINDOW_TOL * 2
        && (a.3 - b.3).abs() <= EDITABLE_PROXY_WINDOW_TOL * 2;
    same_origin
        || rect_contains_rect_with_margin(a, b, 80, 80)
        || rect_contains_rect_with_margin(b, a, 80, 80)
}

fn build_proxy_bottom_band(
    hint: &crate::platform::EditableFocusHintInfo,
) -> Option<(i32, i32, i32, i32)> {
    if !editable_rect_has_area(hint.editor) {
        return None;
    }
    let reference = if rect_contains_rect_with_margin(hint.pane, hint.editor, 64, 128) {
        hint.pane
    } else if rect_contains_rect_with_margin(hint.window, hint.editor, 64, 128) {
        hint.window
    } else {
        return None;
    };
    let side_pad = (hint.editor.2 / 3).max(96).min(140);
    let top_pad = (hint.editor.3 * 3)
        .max(96)
        .min((reference.3 * 18 / 100).max(120));
    let bottom_pad = (hint.editor.3 * 2)
        .max(80)
        .min((reference.3 * 14 / 100).max(96));
    let band_left = (reference.0 + 8).max(hint.editor.0 - side_pad);
    let band_right = (reference.0 + reference.2 - 8)
        .min(hint.editor.0 + hint.editor.2 + side_pad);
    let band_top = (reference.1 + 8).max(hint.editor.1 - top_pad);
    let band_bottom = (reference.1 + reference.3 - 8)
        .min(hint.editor.1 + hint.editor.3 + bottom_pad);
    let band = (
        band_left,
        band_top,
    ...
    if editable_rect_has_area(band) {
        Some(band)
    } else {
        None
    }
...
fn proxy_hit_test(
    ...
    let hit_editor = rect_contains_point_with_margin(hint.editor, x, y, EDITABLE_PROXY_WINDOW_TOL, EDITABLE_PROXY_WINDOW_TOL);
    let proxy_band = build_proxy_bottom_band(hint);
    let hit_band = proxy_band
...
fn take_recent_editable_click_intent() -> Option<EditableClickIntent> {
    let mut lock = RECENT_EDITABLE_CLICK_INTENT.lock().unwrap();
    match lock.as_ref() {
        Some(intent) if intent.created_at.elapsed() <= Duration::from_millis(EDITABLE_CLICK_OBSERVE_MS) => {
            Some(intent.clone())
        }
        Some(_) => {
            *lock = None;
...
fn prune_recent_editable_hints(state: &mut StateEditableFocus) {
    state.recent_editable_hints.retain(|entry| {
        entry.updated_at.elapsed() <= Duration::from_millis(RECENT_EDITABLE_HINT_TTL_MS)
            && editable_rect_has_area(entry.hint.window)
            && editable_rect_has_area(entry.hint.editor)
    });
    if state.recent_editable_hints.len() > RECENT_EDITABLE_HINT_LIMIT {
        state
            .recent_editable_hints
            .sort_by(|left, right| right.updated_at.cmp(&left.updated_at));
        state.recent_editable_hints.truncate(RECENT_EDITABLE_HINT_LIMIT);
    }
}

fn remember_recent_editable_hint(
    state: &mut StateEditableFocus,
    hint: &crate::platform::EditableFocusHintInfo,
) {
    if !hint.editable || !editable_rect_has_area(hint.window) || !editable_rect_has_area(hint.editor) {
        return;
    }
    let now = Instant::now();
    if let Some(existing) = state
        .recent_editable_hints
        .iter_mut()
        .find(|entry| editable_windows_match(entry.hint.window, hint.window))
    {
        existing.hint = hint.clone();
        existing.updated_at = now;
    } else {
        state.recent_editable_hints.push(RecentEditableHint {
            hint: hint.clone(),
            updated_at: now,
        });
    }
...
fn find_recent_editable_hint_for_window(
    state: &StateEditableFocus,
    window: (i32, i32, i32, i32),
...
fn resolve_proxy_anchor_window(
    hint_window: (i32, i32, i32, i32),
    intent_window: (i32, i32, i32, i32),
) -> Option<((i32, i32, i32, i32), &'static str)> {
    if editable_rect_has_area(hint_window) {
        Some((hint_window, "hint"))
    } else if editable_rect_has_area(intent_window) {
...
#[inline]
fn is_left_mouse_down(evt: &MouseEvent) -> bool {
...
impl super::service::Reset for StateEditableFocus {
    fn reset(&mut self) {
        *self = Default::default();
    }
}
...
pub fn new_editable_focus() -> GenericService {
    let svc = EmptyExtraFieldService::new(NAME_EDITABLE_FOCUS.to_owned(), false);
    // Poll at ~50 ms when active, service::repeat interval is 50 ms.
    GenericService::repeat::<StateEditableFocus, _, _>(&svc.clone(), 50, run_editable_focus);
    svc.sp
}

fn hint_changed(prev: &Option<crate::platform::EditableFocusHintInfo>,
                next: &crate::platform::EditableFocusHintInfo) -> bool {
    match prev {
        None => true,
        Some(p) => {
            // First check: foreground window HWND changed (detects app switch
            // even when two maximized windows have identical rects).
            if p.foreground_hwnd != 0 && next.foreground_hwnd != 0
                && p.foreground_hwnd != next.foreground_hwnd {
                return true;
            }
            // Detect state change with a 4-pixel threshold for geometry
            if p.editable != next.editable || p.content_kind != next.content_kind
                || p.display_idx != next.display_idx {
                return true;
            }
            if !p.editable && !next.editable {
                // When both are non-editable, still detect foreground window changes
                // so the mobile side can trigger Window Auto-Viewport on app switch.
                let rect_changed = |a: (i32, i32, i32, i32), b: (i32, i32, i32, i32)| -> bool {
                    (a.0 - b.0).abs() > 8 || (a.1 - b.1).abs() > 8
                        || (a.2 - b.2).abs() > 8 || (a.3 - b.3).abs() > 8
                };
                return rect_changed(p.window, next.window);
            }
            let rect_changed = |a: (i32, i32, i32, i32), b: (i32, i32, i32, i32)| -> bool {
                (a.0 - b.0).abs() > 4 || (a.1 - b.1).abs() > 4
                    || (a.2 - b.2).abs() > 4 || (a.3 - b.3).abs() > 4
            };
            rect_changed(p.caret, next.caret)
                || rect_changed(p.editor, next.editor)
                || rect_changed(p.window, next.window)
                || rect_changed(p.pane, next.pane)
        }
    }
}

fn window_rect_changed(
    prev: (i32, i32, i32, i32),
    next: (i32, i32, i32, i32),
    tol: i32,
) -> bool {
    (prev.0 - next.0).abs() > tol
        || (prev.1 - next.1).abs() > tol
        || (prev.2 - next.2).abs() > tol
        || (prev.3 - next.3).abs() > tol
}
...
mod tests {
    ...
    fn make_hint(
        ...
        EditableFocusHintInfo {
            editable,
            window,
            editor: (400, 900, 500, 60),
            pane: (400, 60, 1500, 1000),
            caret: (620, 930, 2, 22),
            display_idx: 0,
    ...
    #[test]
    fn hint_changed_detects_noneditable_app_switch_with_same_rect() {
        let prev = make_hint(false, (-9, -9, 1938, 1158), 1001);
        let next = make_hint(false, (-9, -9, 1938, 1158), 2002);

        assert!(hint_changed(&Some(prev), &next));
    }

    #[test]
    fn hint_changed_detects_editable_app_switch_with_same_rect() {
        let prev = make_hint(true, (-9, -9, 1938, 1158), 3003);
        let next = make_hint(true, (-9, -9, 1938, 1158), 4004);

        assert!(hint_changed(&Some(prev), &next));
    }

    #[test]
    fn hint_changed_ignores_identical_noneditable_hint_when_hwnd_matches() {
        let prev = make_hint(false, (-9, -9, 1938, 1158), 5005);
...
fn send_editable_focus_hint(
    ...
    state: &mut StateEditableFocus,
    hint_info: &crate::platform::EditableFocusHintInfo,
) {
    use hbb_common::message_proto::{EditableFocusHint, RectI32};

    state.revision += 1;
    let rev = state.revision;

    let make_rect = |t: (i32, i32, i32, i32)| -> RectI32 {
        let mut r = RectI32::new();
        r.x = t.0;
        r.y = t.1;
        r.width = t.2;
        r.height = t.3;
    ...
    let mut fh = EditableFocusHint::new();
    fh.editable = hint_info.editable;
    fh.caret = hbb_common::protobuf::MessageField::some(make_rect(hint_info.caret));
    fh.editor = hbb_common::protobuf::MessageField::some(make_rect(hint_info.editor));
    fh.window = hbb_common::protobuf::MessageField::some(make_rect(hint_info.window));
    fh.pane = hbb_common::protobuf::MessageField::some(make_rect(hint_info.pane));
    fh.display_idx = hint_info.display_idx;
    fh.content_kind = match hint_info.content_kind {
        1 => hbb_common::message_proto::EditableContentKind::ECK_SINGLE_LINE,
        2 => hbb_common::message_proto::EditableContentKind::ECK_MULTI_LINE,
        3 => hbb_common::message_proto::EditableContentKind::ECK_CODE,
        _ => hbb_common::message_proto::EditableContentKind::ECK_UNKNOWN,
    }
    .into();
    fh.revision = rev;

    let mut misc = Misc::new();
    misc.set_editable_focus_hint(fh);
    let mut msg_out = Message::new();
    msg_out.set_misc(misc);
    sp.send(msg_out);
}
...
fn run_editable_focus(sp: EmptyExtraFieldService, state: &mut StateEditableFocus) -> ResultType<()> {
    // Adaptive polling: slow down when nothing changes (idle = every 3 ticks = 150 ms)
    state.idle_counter = state.idle_counter.saturating_add(1);
    prune_recent_editable_hints(state);
    let recent_click_intent = take_recent_editable_click_intent();
    let active = state.last_hint.as_ref().map(|h| h.editable).unwrap_or(false);
    if !active && recent_click_intent.is_none() && state.idle_counter % 3 != 0 {
        return Ok(()); // idle pace
    }
    ...
    if !hint_info.editable {
                ...
                resolve_proxy_anchor_window(hint_info.window, intent.window)
            {
                    ...
                    find_recent_editable_hint_for_window(state, anchor_window)
                {
                    ...
                    if current_window_match && candidate_window_match && hit_editor {
                        log::debug!(
                            "editable_focus proxy reuse: seq={} anchor={} source={} age_ms={} point=({}, {}) current_window={:?} anchor_window={:?} last_window={:?} editor={:?} pane={:?} proxy_band={:?}",
                            intent.seq,
                            anchor_source,
                            candidate_source,
                            candidate_age_ms,
                        ...
                        log::debug!(
                            "editable_focus proxy miss: seq={} anchor={} source={} age_ms={} point=({}, {}) current_window={:?} anchor_window={:?} last_window={:?} editor={:?} pane={:?} proxy_band={:?} match_current={} match_last={} hit_editor={} hit_band={}",
                            intent.seq,
                            anchor_source,
                            candidate_source,
                            candidate_age_ms,
                    ...
                    log::debug!(
                        "editable_focus proxy miss: seq={} anchor={} source=none point=({}, {}) current_window={:?} anchor_window={:?} reason=no-candidate",
                        intent.seq,
                        anchor_source,
                        intent.x,
                        intent.y,
                ...
                log::debug!(
                    "editable_focus proxy miss: seq={} source=none point=({}, {}) current_window={:?} intent_window={:?} reason=no-window-anchor",
                    intent.seq,
                    intent.x,
                    intent.y,
                    hint_info.window,
        ...
        } else if let Some(anchor_window) = crate::platform::windows::get_foreground_window_rect() {
            if let Some((candidate_hint, candidate_source, candidate_age_ms)) =
                find_recent_editable_hint_for_window(state, anchor_window)
            {
                if let Some((cursor_x, cursor_y)) = crate::get_cursor_pos() {
                    let candidate_window_match =
                        editable_windows_match(candidate_hint.window, anchor_window);
                    let (proxy_band, hit_editor, hit_band) =
                    ...
                    if candidate_window_match && hit_editor {
                        ...
                        log::debug!(
                            "editable_focus cursor proxy miss: source={} age_ms={} point=({}, {}) current_window={:?} anchor_window={:?} last_window={:?} editor={:?} pane={:?} proxy_band={:?} match_last={} hit_editor={} hit_band={}",
                            candidate_source,
                            candidate_age_ms,
                            cursor_x,
                            cursor_y,
    ...
    if let Some(intent) = recent_click_intent.as_ref() {
        if state.last_observed_click_seq != intent.seq {
            log::info!(
                "editable_focus post-click observe: seq={} age_ms={} editable={} kind={} window={:?} editor={:?} pane={:?}",
                intent.seq,
    ...
    // Debug: log every 60th poll (~3 sec) regardless of change
    if state.idle_counter % 60 == 1 {
        log::debug!("editable_focus poll: editable={}, caret={:?}, editor={:?}, window={:?}, kind={}",
            hint_info.editable, hint_info.caret, hint_info.editor, hint_info.window, hint_info.content_kind);
    }

    let foreground_window_changed = state.last_hint.as_ref().map(|prev| {
        window_rect_changed(prev.window, hint_info.window, 8)
    }).unwrap_or(false);

    if !hint_changed(&state.last_hint, &hint_info) {
        // Some apps keep the same editable control focused for a long time.
        // Refreshing the unchanged hint lets reconnecting/mobile clients re-open
        // their IME even when focus geometry never changes again.
        // NOTE: Use 5 (~500ms) instead of 20 (~2s) to ensure the hint arrives
        // within the ARM window when tapping the *same* input field after keyboard close.
        if hint_info.editable && state.idle_counter % 5 == 0 {
            log::debug!("editable_focus REFRESH: editable=true, kind={}, rev={}",
                hint_info.content_kind, state.revision + 1);
    ...
    if let Some(intent) = recent_click_intent.as_ref() {
        log::info!(
            "editable_focus post-click transition: seq={} age_ms={} editable={} kind={} rev={} window={:?} editor={:?} pane={:?}",
            intent.seq,
            intent.created_at.elapsed().as_millis(),
            hint_info.editable,
            hint_info.content_kind,
    ...
    if foreground_window_changed {
        if let Some(prev) = state.last_hint.as_ref() {
            log::info!(
                "editable_focus foreground-window changed: prev_editable={} next_editable={} prev_window={:?} next_window={:?} next_editor={:?} next_pane={:?}",
                prev.editable,
                hint_info.editable,
                prev.window,
                hint_info.window,
...
lazy_static::lazy_static! {
    ...
    static ref RECENT_EDITABLE_CLICK_INTENT: Arc<Mutex<Option<EditableClickIntent>>> = Default::default();
