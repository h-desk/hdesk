on run {daemon_file, agent_file, user, cur_pid, source_dir}
  ...
  set agent_plist to "/Library/LaunchAgents/cn.yunjichuangzhi.hdesk_server.plist"
  set daemon_plist to "/Library/LaunchDaemons/cn.yunjichuangzhi.hdesk_service.plist"
  set app_bundle to "/Applications/HDesk.app"
  ...
  set kill_others to "pids=$(pgrep -x 'HDesk' | grep -vx " & cur_pid & " || true); if [ -n \"$pids\" ]; then echo \"$pids\" | xargs kill -9 || true; fi;"
  ...
  do shell script sh with prompt "HDesk wants to update itself" with administrator privileges
end run
