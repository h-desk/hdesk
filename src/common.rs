...
#[cfg(not(target_os = "ios"))]
fn ipv6_interface_label(interface: &default_net::Interface) -> String {
    interface
        .friendly_name
        .clone()
...
#[cfg(not(target_os = "ios"))]
fn ipv6_interface_text(interface: &default_net::Interface) -> String {
    format!(
        "{} {} {} {}",
        interface.name,
        interface.friendly_name.as_deref().unwrap_or_default(),
        interface.description.as_deref().unwrap_or_default(),
        interface.if_type.name(),
    )
    .to_ascii_lowercase()
}

#[cfg(not(target_os = "ios"))]
fn is_vpn_like_interface(interface: &default_net::Interface) -> bool {
    use default_net::interface::InterfaceType;

    if matches!(
        interface.if_type,
        InterfaceType::Tunnel | InterfaceType::Ppp | InterfaceType::Loopback
    ) {
        return true;
    }

    let text = ipv6_interface_text(interface);
    [
        "vpn",
        "wireguard",
        "tailscale",
...
#[cfg(not(target_os = "ios"))]
fn ipv6_interface_base_score(interface: &default_net::Interface) -> i32 {
    use default_net::interface::InterfaceType;

    let mut score = match interface.if_type {
        InterfaceType::Wireless80211 => 520,
        InterfaceType::GigabitEthernet
        | InterfaceType::Ethernet
        | InterfaceType::FastEthernetT
        | InterfaceType::FastEthernetFx
        | InterfaceType::Ethernet3Megabit => 500,
        InterfaceType::Wwanpp | InterfaceType::Wwanpp2 | InterfaceType::Wman => 420,
        InterfaceType::Tunnel | InterfaceType::Ppp | InterfaceType::Loopback => -1500,
        InterfaceType::Unknown => 120,
        _ => 180,
    };
    ...
    if interface
        .mac_addr
        .as_ref()
        .map(|mac| mac.octets() != [0; 6])
        .unwrap_or(false)
    {
        score += 15;
    }
    if let Some(speed) = interface.receive_speed.or(interface.transmit_speed) {
        score += ((speed / 1_000_000_000).min(5) as i32) * 10;
    }
    if is_vpn_like_interface(interface) {
        score -= 2000;
    }
...
#[cfg(not(target_os = "ios"))]
fn get_ipv6_bind_candidates(preferred_ip: Option<Ipv6Addr>) -> Vec<Ipv6BindCandidate> {
    let mut primary = Vec::new();
    let mut fallback = Vec::new();
    let mut seen = HashSet::new();

    for interface in default_net::get_interfaces() {
        let interface_label = ipv6_interface_label(&interface);
        let is_vpn_like = is_vpn_like_interface(&interface);
        let mut interface_score = ipv6_interface_base_score(&interface);
        if let Some(preferred_ip) = preferred_ip {
            if interface.ipv6.iter().any(|ipv6| ipv6.addr == preferred_ip) {
                interface_score += 400;
            }
        }

        for ipv6 in &interface.ipv6 {
            if !seen.insert(ipv6.addr) || !is_public_ipv6(&ipv6.addr) {
                continue;
            }

            let mut score = interface_score;
            if Some(ipv6.addr) == preferred_ip {
                score += 600;
            }
            if ipv6.prefix_len == 64 {
                score += 20;
            }

            let candidate = Ipv6BindCandidate {
                bind_addr: SocketAddr::from((ipv6.addr, 0)),
                interface_label: interface_label.clone(),
                score,
            };
            if is_vpn_like {
    ...
    primary.sort_by(|a, b| {
        b.score
            .cmp(&a.score)
            .then_with(|| a.interface_label.cmp(&b.interface_label))
    });
...
#[cfg(not(target_os = "ios"))]
async fn stun_ipv6_test_with_bind_addr(
    bind_addr: SocketAddr,
    stun_server: &str,
) -> ResultType<(SocketAddr, SocketAddr, String)> {
    use std::net::ToSocketAddrs;
    use stunclient::StunClient;
    ...
    let addr = client.query_external_address_async(&socket).await?;
    Ok(if addr.ip().is_ipv6() {
        (local_addr, addr, stun_server.to_owned())
    } else {
        bail!("STUN server returned non-IPv6 address: {}", addr)
    })
}
...
#[cfg(not(target_os = "ios"))]
async fn select_validated_ipv6_candidate(
    candidates: &[Ipv6BindCandidate],
) -> Option<(SocketAddr, String, String)> {
    for candidate in candidates {
        for &stun in &STUNS_V6 {
            match stun_ipv6_test_with_bind_addr(candidate.bind_addr, stun).await {
                Ok((mut local_addr, external_addr, stun_server)) => {
                    match (local_addr.ip(), external_addr.ip()) {
                        (IpAddr::V6(local_ip), IpAddr::V6(external_ip))
                            if local_ip == external_ip =>
                        {
                            local_addr.set_port(0);
                            return Some((
                                local_addr,
                                candidate.interface_label.clone(),
                                stun_server,
                            ));
                        }
                        (IpAddr::V6(local_ip), IpAddr::V6(external_ip)) => {
                            log::warn!(
                                "Ignoring IPv6 candidate {} on {} because STUN observed {} via {}",
                                local_ip,
                                candidate.interface_label,
                                external_ip,
                                stun_server
                            );
                        }
                ...
                Err(err) => {
                    log::debug!(
                        "IPv6 candidate {} on {} failed STUN validation via {}: {}",
                        candidate.bind_addr,
                        candidate.interface_label,
                        stun,
                        err
...
pub async fn test_ipv6() -> Option<tokio::task::JoinHandle<()>> {
    ...
    let preferred_local_addr = match test_bind_ipv6().await {
        Ok(mut addr) => {
    ...
    #[cfg(not(target_os = "ios"))]
    let candidates =
        get_ipv6_bind_candidates(preferred_local_addr.and_then(|addr| match addr.ip() {
    ...
    if let Some(candidate) = candidates.first() {
        PUBLIC_IPV6_ADDR.lock().unwrap().0 = Some(candidate.bind_addr);
        log::info!(
            "Prefer IPv6 candidate {} on {}",
            candidate.bind_addr,
            candidate.interface_label
        );
    } else if let Some(addr) = preferred_local_addr {
    ...
    #[cfg(not(target_os = "ios"))]
    if candidates.is_empty() && preferred_local_addr.is_some() {
        return None;
    }

    #[cfg(target_os = "ios")]
    if preferred_local_addr.is_some() {
        return None;
    }

    Some(tokio::spawn(async move {
        #[cfg(not(target_os = "ios"))]
        if let Some((addr, interface_label, stun_server)) =
            select_validated_ipv6_candidate(&candidates).await
        {
            PUBLIC_IPV6_ADDR.lock().unwrap().0 = Some(addr);
            log::info!(
                "Validated IPv6 candidate {} on {} via {}",
                addr,
                interface_label,
                stun_server
            );
        ...
        if !candidates.is_empty() {
            log::warn!("Keeping provisional IPv6 candidate because STUN validation did not confirm a better one");
            return;
        }
