...
fn bindgen_builder() -> bindgen::Builder {
    ...
    #[cfg(windows)]
    {
        return add_windows_bindgen_includes(builder);
    }
    #[cfg(not(windows))]
    {
        builder
...
#[cfg(windows)]
fn add_windows_bindgen_includes(mut builder: bindgen::Builder) -> bindgen::Builder {
    builder = builder.clang_arg("--target=x86_64-pc-windows-msvc");
    for dir in windows_bindgen_include_dirs() {
...
#[cfg(windows)]
fn windows_bindgen_include_dirs() -> Vec<PathBuf> {
    let mut include_dirs = Vec::new();

    if let Some(dir) = clang_resource_include_dir() {
        include_dirs.push(dir);
    }
    if let Some(dir) = msvc_include_dir() {
        include_dirs.push(dir);
    }
    include_dirs.extend(windows_sdk_include_dirs());
...
fn clang_resource_include_dir() -> Option<PathBuf> {
    ...
    let llvm_root = PathBuf::from(libclang_path).parent()?.to_path_buf();
    let clang_root = llvm_root.join("lib").join("clang");
    latest_subdir(&clang_root).map(|dir| dir.join("include"))
}

#[cfg(windows)]
fn msvc_include_dir() -> Option<PathBuf> {
    if let Some(dir) = env::var_os("VCToolsInstallDir") {
    ...
    let msvc_root = PathBuf::from(program_files_x86)
        ...
        .join("Tools")
        .join("MSVC");
    latest_subdir(&msvc_root).map(|dir| dir.join("include"))
}
...
fn windows_sdk_include_dirs() -> Vec<PathBuf> {
    ...
    let sdk_include_root = env::var_os("WindowsSdkDir")
        .map(PathBuf::from)
        .map(|dir| dir.join("Include"))
        .or_else(|| {
            env::var_os("ProgramFiles(x86)").map(|base| {
                PathBuf::from(base)
                    .join("Windows Kits")
                    .join("10")
                    .join("Include")
    ...
    let Some(sdk_include_root) = sdk_include_root else {
        return include_dirs;
    };

    let version_dir = env::var_os("WindowsSDKVersion")
        .map(|version| sdk_include_root.join(version))
        .filter(|dir| dir.exists())
        .or_else(|| latest_subdir(&sdk_include_root));
