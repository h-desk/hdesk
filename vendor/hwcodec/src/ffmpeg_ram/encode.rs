use crate::{
    common::{
        DataFormat, Quality, RateControl, TEST_TIMEOUT_MS, AV1, H264, H265, VP8, VP9,
    },
    ffmpeg::{init_av_log, AVPixelFormat, AV_PIX_FMT_YUV420P},
    ffmpeg_ram::{
...
impl Encoder {
    ...
    pub fn available_encoders(ctx: EncodeContext, _sdk: Option<String>) -> Vec<CodecInfo> {
        ...
        codecs.retain(|c| {
            let ctx = ctx.clone();
            if ctx.pixfmt == AV_PIX_FMT_YUV420P && c.name.contains("qsv") {
                return false;
            }
            return true;
        });
